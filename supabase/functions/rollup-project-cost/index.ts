...
async function handler(req: Request, supabase: any) {
  ...
  const { data, error } = await supabase.rpc('noop'); // placeholder for real rollup
  // For demo, query aggregate directly
  const { data: agg } = await supabase.from('cost.timesheets')
    .select('hours, rate_snapshot, bill_rate_snapshot, project_id')
    .eq('project_id', project_id);
  let total_hours = 0, total_cost = 0, total_bill = 0;
  for (const r of agg ?? []) {
    total_hours += Number(r.hours || 0);
    total_cost += Number(r.hours || 0) * Number(r.rate_snapshot || 0);
    total_bill += Number(r.hours || 0) * Number(r.bill_rate_snapshot || 0);
  }
  return { total_hours, total_cost, total_bill };
}
