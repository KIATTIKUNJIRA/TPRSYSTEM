...
async function handler(req: Request, supabase: any) {
  ...
  if (!project_id || !title) throw new Error("project_id/title required");
  const { data, error } = await supabase.from('proj.board_items').insert({ project_id, title }).select().single();
